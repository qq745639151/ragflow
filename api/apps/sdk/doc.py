...
async def update_doc(tenant_id, dataset_id, document_id):
    ...
    if "chunk_method" in req:
        valid_chunk_method = {"naive", "custom", "custom_paragraph", "manual", "qa", "table", "paper", "book", "laws", "presentation", "picture", "one", "knowledge_graph", "email", "tag"}
...
@manager.route("/retrieval_new", methods=["POST"])  # noqa: F821
@token_required
async def retrieval_new(tenant_id):
    """
    Retrieve chunks based on a query with token limit validation.
    ---
    tags:
      - Retrieval
    security:
      ...
      200:
        ...
        schema:
          ...
          properties:
            ...
            total_tokens:
              type: integer
              description: Total number of tokens in the result.
    """
    req = await get_request_json()
    if not req.get("dataset_ids"):
        return get_error_data_result("`dataset_ids` is required.")
    ...
    try:
        ...
        if max_tokens and isinstance(max_tokens, int) and max_tokens > 0:
            ...
            truncated_chunks = []
            for chunk in ranks["chunks"]:
                # Calculate tokens for chunk content (consistent with kb_prompt logic)
                chunk_text = chunk.get("content", "")
                chunk_tokens = num_tokens_from_string(chunk_text)
                
                # Check if adding this chunk would exceed the limit (using 97% of max_tokens as threshold)
                if total_tokens + chunk_tokens <= max_tokens * 0.97:
                    truncated_chunks.append(chunk)
                    total_tokens += chunk_tokens
                else:
                    break
            
            ranks["chunks"] = truncated_chunks
            ranks["total_tokens"] = total_tokens
