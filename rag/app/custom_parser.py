...
def _describe_timeout_value(value, *, timeout_controls_enabled):
    if value is not None:
        return f"{value:g}s"
    if timeout_controls_enabled:
        return "disabled"
    return "httpx default"


_DEFAULT_XIDIAN_CONNECT_TIMEOUT = 30.0
_DEFAULT_XIDIAN_WRITE_TIMEOUT = 300.0
_DEFAULT_XIDIAN_READ_TIMEOUT = 3600.0


def _timeout_controls_enabled(parser_config):
    raw = parser_config.get("xidian_enable_timeout")
    if raw is None:
        return False  # Default: disable timeout (no limit)
...
def _fetch_xidian_payload(filename, binary, parser_config, callback=None):
    api_url = (
        os.getenv("EXTERNAL_PREPROCESS_URL", "http://124.70.213.108:9431/api/v1/xidian/preprocess_required")
    )
    enable_timeout = _timeout_controls_enabled(parser_config)
    timeout = (
        _optional_timeout_value(
            parser_config.get("xidian_timeout"),
            default=_DEFAULT_XIDIAN_READ_TIMEOUT,
        )
        if enable_timeout else None
    )
    connect_timeout = (
        _optional_timeout_value(
            parser_config.get("xidian_connect_timeout"),
            default=_DEFAULT_XIDIAN_CONNECT_TIMEOUT,
        )
        if enable_timeout else None
    )
    write_timeout = (
        _optional_timeout_value(
            parser_config.get("xidian_write_timeout"),
            default=_DEFAULT_XIDIAN_WRITE_TIMEOUT,
        )
        if enable_timeout else None
    )
    ...
    read_timeout_desc = _describe_timeout_value(timeout, timeout_controls_enabled=enable_timeout)
    connect_timeout_desc = _describe_timeout_value(connect_timeout, timeout_controls_enabled=enable_timeout)
    ...
    try:
        ...
        if enable_timeout:
            ...
            # Disable all timeouts when timeout controls are disabled
        ...
        raise RuntimeError(
            f"Xidian preprocess API read timeout after {read_timeout_desc}. "
        ...
        raise RuntimeError(
            f"Xidian preprocess API connect timeout after {connect_timeout_desc}. "
            f"api_url={api_url}, trust_env={trust_env}. "
            "Check service reachability or increase xidian_connect_timeout."
        ) from exc
...
def _apply_chunk_title(d, heading_path):
    ...
    if not heading_path:
        return
    d["title_tks"] = rag_tokenizer.tokenize(heading_path)
...
def _doc_positions_to_raw(doc):
    positions = []
    for pos in doc.get("position_int") or []:
        if len(pos) != 5:
            continue
        page_num, left, right, top, bottom = pos
        positions.append((max(int(page_num) - 1, 0), int(left), int(right), int(top), int(bottom)))
    return positions


def _set_media_count(media_counts, chunk, doc_type, count=1):
    media_counts[id(chunk)][doc_type] = count


def _media_count(media_counts, chunk, doc_type):
    return media_counts.get(id(chunk), {}).get(doc_type, 0)


def _append_media_doc(results, media_counts, doc, content, eng, positions, doc_type):
    d = _build_media_doc(doc, content, eng, positions, doc_type)
    results.append(d)
    _set_media_count(media_counts, d, doc_type)


def _append_media_to_previous(results, media_counts, doc, content, eng, positions, doc_type):
    content = _clean_text(content)
    if not content:
        return
    if not results or _media_count(media_counts, results[-1], doc_type) >= 1:
        _append_media_doc(results, media_counts, doc, content, eng, positions, doc_type)
        return

    previous = results[-1]
    previous_content = _clean_text(previous.get("content_with_weight"))
    merged_content = f"{previous_content}\n\n{content}".strip() if previous_content else content
    tokenize(previous, merged_content, eng)
    previous["doc_type_kwd"] = doc_type
    _set_media_count(media_counts, previous, doc_type)

    merged_positions = _merge_positions([_doc_positions_to_raw(previous), positions])
    if merged_positions:
        add_positions(previous, merged_positions)


def _format_table_content(caption, table_html, image_path):
    parts = []
    if caption:
...
def custom_parse(filename, binary=None, from_page=0, to_page=100000, lang="Chinese", callback=None, **kwargs):
    ...
    for index, part in enumerate(partitions):
        ...
        if part_type == "table":
            flush_section()
            table_text = _format_table_content(caption, data, _clean_text(part.get("image_path")))
            if table_text:
                if caption:
                    _append_media_doc(results, media_counts, doc, table_text, eng, positions, "table")
                else:
                    _append_media_to_previous(results, media_counts, doc, table_text, eng, positions, "table")
            continue
        ...
        if part_type in {"image", "chart"}:
            ...
            if media_text:
                _append_media_to_previous(
                    ...
                    eng,
                    positions,
                    "image" if part_type == "image" else "chart",
                )
            continue
